...
public class AbstractRestRouteBuilder extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		onException(SocketException.class)
				.handled(true)
				.maximumRedeliveries(0)
				.log(LoggingLevel.ERROR, "Error for correlation : ${in.header.correlation} ,  SocketException with error body ==> " + exceptionMessage())
				.to(String.format("direct:%s", Routes.TIME_OUT_EXCEPTION_ROUTE));
		
		onException(SocketTimeoutException.class)
				.handled(true)
				.maximumRedeliveries(0)
				.log(LoggingLevel.ERROR, "Error for correlation : ${in.header.correlation} , SocketTimeoutException with error body ==> " + exceptionMessage())
				.to(String.format("direct:%s", Routes.TIME_OUT_EXCEPTION_ROUTE));
